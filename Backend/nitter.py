...
class Nitter:
    ...
    def get_replies(
        ...
        while keep_scraping:
            thread = []
            for tweet in soup.find_all("div", class_="reply"):
                if len(tweet["class"]) == 1:
                    to_append = self._extract_tweet(tweet, is_encrypted)
                    # Extract tweets
                    if len(tweets["tweets"]) + len(tweets["threads"]) < number:
                    ...
                    if "thread" in tweet["class"]:
                        ...
                        if len(tweet["class"]) == 3:
                            tweets["threads"].append(to_append)
                            thread = []
            ...
            if (
                ...
                # Go to the next page
                show_more_buttons = soup.find_all("div", class_="show-more")
                if soup.find_all("div", class_="show-more", string="Load more"):
                    next_page = endpoint + show_more_buttons[-1].find("a")["href"]
                    soup = self._get_page(next_page, max_retries)
                    if soup is None:
                        keep_scraping = False
                else:
