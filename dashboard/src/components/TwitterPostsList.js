...
const columns = [
    ...
    },
    {
      ...
      format: (value) =>
        ...
        {value.map((tag) => tag === "none" || tag === "" ? <></> : <span className="post--tag">{tag}</span>)}            </div>); } ,
    },
    {
      id: 'numComments',
      label: 'Comments',
                ...
                <TableBody>
                    ...
                    .map( (comment) => (
                        <TableRow>
                            ...
                            <TableCell sx = {{"font-family": "Montserrat, sans-serif"}}>{comment.text}</TableCell>
...
const PostsList = ({settings}) => {
    ...
    useEffect(() => {
        axios.post(`${process.env.REACT_APP_API_BASE_URL}/twitter-posts`, {account: settings["twitterName"]})
      .then(res => {
        setPosts(res.data);
    ...
    const filterPosts = () => {
      setPosts( allPosts.filter( 
        (value, index, array) => {
          ...
          let dateMatch = (dateFilter1 <= date && date <= dateFilter2) || !date;
          let sentimentMatch = sentimentFilter1 <= sentiment && sentiment <= sentimentFilter2;
          return topicMatch && emotionMatch && dateMatch && sentimentMatch;} ) )
    }
                    ...
                    >
                      <Table>
                        ...
                        <TableRow>
                          ...
                          <TableCell>
                            <TextField size="small" onChange={(e) => setEmotionFilter(e.target.value)}/>
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell>
                ...
                <TableHead sx={{
                  [`& .${tableCellClasses.root}`]: {
                    ...
                    {columns.map((column) => (
                        ...
                        sx = {{"font-family": "Montserrat, sans-serif", "font-weight": "700", "background-color": "var(--primary2)"}}
                        >
                        {column.label}
                        </TableCell>
                    ))}
                    <TableCell sx = {{"font-family": "Montserrat, sans-serif", "font-weight": "700", "background-color": "var(--primary2)"}}>
